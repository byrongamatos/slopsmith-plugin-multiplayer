...
- **Session-end finalization.** When either endpoint's grace expires, OR a connection arrives with the **same** `player_id` but a **different** `session_id` (a real takeover), the server finalizes the session: closes any still-connected endpoint with `1011`, clears `broadcaster_id`, emits `broadcaster_changed: null` to all peers, and discards the held record. The takeover case then proceeds with the new session normally.
