...
**Status (as of Phase 1c).** The `session_id` parameter, the per-session takeover/grace lifecycle, the lifecycle close codes `4401` / `4408` / `4409` / `4410`, the audio WS endpoint with binary fan-out and `SMAU` header validation (`1009` on size violation), AND the broadcast control plane (`broadcast_start` / `broadcast_stop` / `broadcaster_changed` / `broadcaster_busy` / `audio_quality`) plus `broadcaster_id` + `broadcast_params` room state are all implemented **server-side** in `routes.py` and exercised by `tests/test_session_lifecycle.py` + `tests/test_audio_ws.py` + `tests/test_broadcast_control.py`. Browser-side wiring in `screen.js` for `/audio` WebSocket and the broadcast control panel is still pending — that lands in a later wave.
