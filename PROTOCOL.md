...
**Status (as of Phase 1b).** The `session_id` parameter, the per-session takeover/grace lifecycle, the lifecycle close codes `4401` / `4408` / `4409` / `4410`, the audio WS endpoint, the binary frame format with server-side header validation (`SMAU` magic, version, length consistency, `MAX_FRAME_BYTES`), and `1009` close-on-size-violation are all implemented in `routes.py` + `screen.js` and exercised by the test suites under `tests/test_session_lifecycle.py` (highway side) and `tests/test_audio_ws.py` (audio side, binary fan-out, header validation, cross-endpoint session lifecycle). The broadcast control plane (`broadcast_start` / `broadcaster_changed` / `audio_quality` / `broadcaster_busy`) is still pending — that lands in Phase 1c, along with `broadcaster_id` room state.
