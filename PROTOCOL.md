...
- **Session-end finalization.** When either endpoint's grace expires, OR a connection arrives with the **same** `player_id` but a **different** `session_id` (a real takeover), the server finalizes the session: closes any still-connected endpoint with `4409` (the same lifecycle close code already used for takeovers — clients receiving `4409` already know to tear down both endpoints, so reusing it covers both grace-expiry and takeover with one signal), clears `broadcaster_id`, emits `broadcaster_changed: null` to all peers, and discards the held record. The takeover case then proceeds with the new session normally. `1011` is reserved exclusively for unexpected server errors (the close-code table above) and is NEVER emitted for ordinary session lifecycle events.
