...
**Status (as of Phase 2b).** Server-side: `session_id` + per-session takeover/grace lifecycle (`4401` / `4408` / `4409` / `4410`) + `/audio` endpoint with binary fan-out and `SMAU` header validation (`1009`) + broadcast control plane (`broadcast_start` / `broadcast_stop` / `broadcaster_changed` / `broadcaster_busy` / `audio_quality`) + `broadcaster_id` / `broadcast_params` room state. Exercised by `tests/test_session_lifecycle.py` + `tests/test_audio_ws.py` + `tests/test_broadcast_control.py`. Client-side: `screen.js` opens `/audio` alongside the highway WS using the same `session_id`, handles all lifecycle close codes (4401 / 4408 / 4409 / 4410 / 1009 / 1011) with independent per-endpoint reconnect cadence, parses the 40-byte SMAU header on every inbound binary frame and accounts valid frames + drop reasons (`audio/smau-frame.js` is the source of truth, exercised by `audio/smau-frame.test.js`; `screen.js` carries an inlined duplicate because the plugin loader serves only the single `screen.js` entry point), and tears both endpoints down cleanly on leave/cleanup. The actual audio capture/playback pipelines (broadcast UI, getUserMedia, AudioWorklet, WebCodecs Opus encode/decode, AudioBufferSourceNode scheduling) still pending — listener decode + scheduling lands in Phase 2c, broadcast capture + UI in Phase 2d.
