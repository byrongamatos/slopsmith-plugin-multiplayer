...
**Status (as of Phase 1b).** The `session_id` parameter, the per-session takeover/grace lifecycle, and the lifecycle close codes `4401` / `4408` / `4409` / `4410` are implemented in `routes.py` + `screen.js` and exercised by `tests/test_session_lifecycle.py`. The audio WS endpoint, the binary frame format with server-side header validation (`SMAU` magic, version, length consistency, `MAX_FRAME_BYTES`), and `1009` close-on-size-violation are implemented **server-side** in `routes.py` and exercised by `tests/test_audio_ws.py` (binary fan-out, header validation, cross-endpoint session lifecycle); browser-side `/audio` WebSocket wiring in `screen.js` is still pending. The broadcast control plane (`broadcast_start` / `broadcaster_changed` / `audio_quality` / `broadcaster_busy`) is still pending — that lands in Phase 1c, along with `broadcaster_id` room state.
