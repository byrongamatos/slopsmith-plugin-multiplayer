...
The two close codes carry the entire required signal — clients MUST branch on `event.code` (`4409` vs `4410`), not on `event.reason`, since some WebSocket stacks normalize or drop the reason string.

**Client behavior on close:**
- **`4409` (superseded by another tab):** the user's own action elsewhere has invalidated this session. Both endpoints are or will be closed by the server. Clients MUST tear down BOTH the highway-WS and audio-WS state cleanly. UI SHOULD show a non-error notice ("Your session moved to another tab") rather than an error.
...
- **Active state.** While at least one of the two endpoints (highway or audio) is open, the session is alive. No `broadcaster_changed: null` is emitted on individual endpoint disconnects, regardless of whether the close was `4410` (server-initiated slot replace), `1006` (abnormal closure / network drop), `1001` (going away), or anything else.
- **Reconnecting state.** When BOTH endpoints have closed (in any combination of orders, any close codes), the session enters a server-side `RECONNECTING` state for `SESSION_GRACE_PERIOD_SEC` (RECOMMENDED: `5` seconds). During this window the server holds the session record (`broadcaster_id`, codec params) but emits no events to peers.
- **Reattach.** If a new connection arrives during the grace period with the **same** `session_id` and `player_id`, the server reattaches it to the held session record. No `broadcaster_changed: null` was ever emitted; from listeners' perspective the broadcast continues uninterrupted (they may just observe a brief gap in incoming audio frames, which their jitter buffer absorbs as it would any short network glitch).
- **Session-end finalization.** If the grace period expires with no reattach, OR if a connection arrives with the **same** `player_id` but a **different** `session_id` (a real takeover), the server finalizes the session: clears `broadcaster_id`, emits `broadcaster_changed: null` to all peers, and discards the held record. The takeover case then proceeds with the new session normally.
...
In short: server emits `broadcaster_changed: null` exactly once per real broadcast end. A `4410` close, an abnormal disconnect followed by reattach, and any other transient interruption do not produce a stop event. Only takeover by a different `session_id` or expiry of the reconnect grace period without reattach does.
